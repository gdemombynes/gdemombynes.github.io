...
(function () {
  ...
  function barChart(f) {
    const pts = f.series[0].points, W = 640, rowH = 34, left = 190, right = 60, top = 8;
    ...
    pts.forEach((p, i) => {
      ...
      s.appendChild(el("text", { x: left - 12, y: y + 21, "text-anchor": "end", class: "lab" }, p[0]));
      s.appendChild(el("rect", { x: left, y: y + 8, width: w, height: 18, class: i === pts.length - 1 ? "s1" : "s0" }));
      s.appendChild(el("text", { x: left + w + 8, y: y + 21, class: "val" }, fmt(p[1])));
    });
  ...
  function lineChart(f) {
    const pts = f.series[0].points, W = 640, H = 300, L = 48, R = 30, T = 18, B = 40;
    const xs = pts.map(p => p[0]), ys = pts.map(p => p[1]);
    const x0 = Math.min(...xs), x1 = Math.max(...xs), ymax = Math.ceil(Math.max(...ys) / 20) * 20;
  ...
  function groupedBar(f, opts) {
    const cats = f.categories, ser = f.series, W = 640, H = 300, L = 48, R = 20, T = 30, B = 60;
    ...
    cats.forEach((c, ci) => {
      ...
      if (c.length > 16 && words.length > 1) {
        ...
        t.appendChild(el("tspan", { x: gx, dy: 14 }, words.slice(half).join(" ")));
      } else t.textContent = c;
      s.appendChild(t);
    });
    // legend
    ser.forEach((sv, si) => {
      const lx = L + si * 210, ly = 12;
      const cls = opts && opts.redSecond && si === 1 ? "s-red" : (si === 0 ? "s0" : "s1");
      s.appendChild(el("rect", { x: lx, y: ly - 9, width: 12, height: 12, class: cls }));
      s.appendChild(el("text", { x: lx + 18, y: ly + 1, class: "legend" }, sv.label));
    });
    return s;
  }
  ...
  function careerTimeline(exp) {
    const y0 = 2003, y1 = 2027, W = 900, rowH = 26, L = 8, R = 8, T = 26;
    ...
    rows.forEach((e, i) => {
      ...
      s.appendChild(el("rect", { x: start, y: y + 6, width: w, height: 12, class: "bar" + (i % 2 ? " alt" : "") }));
      const label = (e.n ? e.n + " " : "") + e.city + " · " + e.role;
      const fits = w > label.length * 6.4;
      s.appendChild(el("text", { x: fits ? start + 6 : (end + 8 > W - 230 ? start - 8 : end + 8), y: y + 16, class: "lbl", "text-anchor": fits ? "start" : (end + 8 > W - 230 ? "end" : "start"), fill: fits ? "var(--paper)" : null }, label));
    });
    return s;
  }
  ...
  function heroPlate(places) {
    ...
    places.filter(p => !p.regional).forEach(p => {
      ...
      g.appendChild(el("text", { x: x + 10, y: y + 4, class: "city" }, p.city));
      s.appendChild(g);
    });
